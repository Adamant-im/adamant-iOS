//
//  NativeCoreTests.swift
//  AdamantTests
//
//  Created by Anokhov Pavel on 11/11/2018.
...
class NativeCoreTests: XCTestCase {
    ...
    func testEncodeMessage() {
        ...
        guard let decoded = core.decodeMessage(rawMessage: encoded.message, rawNonce: encoded.nonce, senderPublicKey: aPublicKey, privateKey: bPrivateKey)
        else {
            XCTFail()
            return
        }
    ...
    func testDecodeJsEncodedMessage() {
        let message = "common"
        let aPublicKey = "8007a01493bb4b21ec67265769898eb19514d9427bd7b701f96bc9880a6e209f"
        let aPrivateKey = "9001490b166816af75a15a3e2b0174bfe3be3dfaa63147b4f780ed3ab90ffeab8007a01493bb4b21ec67265769898eb19514d9427bd7b701f96bc9880a6e209f"
        let bPublicKey = "9f895a201fd92cc60ef02d2117d53f00dc2981903cb64b2f214777269b882209"
        ...
        guard let js = Bundle(for: type(of: self)).url(forResource: "adamant-core", withExtension: "js") else {
            fatalError("Can't load system resources!")
        }

        let jsCore = JSAdamantCore()
        jsCore.loadJs(from: js, queue: DispatchQueue.global(qos: .utility)) { (result) in
        ...
        // Encode with JS
        guard let encoded = jsCore.encodeMessage(message, recipientPublicKey: bPublicKey, privateKey: aPrivateKey) else {
            XCTFail()
            return
        }
    ...
    func testEncodeMessageForJs() {
        ...
        // Decode with JS
        guard let decoded = jsCore.decodeMessage(rawMessage: encoded.message, rawNonce: encoded.nonce, senderPublicKey: aPublicKey, privateKey: bPrivateKey)
        else {
            XCTFail()
            return
        }
