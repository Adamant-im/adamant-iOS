...
final class AdmWalletServiceTests: XCTestCase {
  ...
  func test_sendMoney_invalidRecipientThrowsError() async throws {
    ...
    setupAccountService()
    await MainActor.run {
      accountsProviderMock.given(
        .getAccount(byAddress: .any, willThrow: AccountsProviderError.notFound(address: "")))
    }
  ...
  func test_sendMoney_emptyRecipientChatroomThrowsError() async throws {
    ...
    await MainActor.run {
      accountsProviderMock.given(
        .getAccount(byAddress: .any, willReturn: createCoreDataAccount(publicKey: "public key")))
    }
  ...
  func test_sendMoney_correctRecipientBadMessageEncodeThrowsError() async throws {
    // GIVEN
    setupAccountService()
    let (room, account) = setupCoreDataEntities(
      accountPublicKey: Constants.recipientPublicKeyAddress)
    await MainActor.run {
      accountsProviderMock.given(.getAccount(byAddress: .any, willReturn: account))
    }
    adamantCoreMock.given(
      .encodeMessage(.any, recipientPublicKey: .any, privateKey: .any, willReturn: nil))
  ...
  func test_sendMoney_signTransactionFailureThrowsError() async throws {
    ...
    adamantCoreMock.given(
      .encodeMessage(
        .any, recipientPublicKey: .any, privateKey: .any, willReturn: ("message", "nonce")))
  ...
  func test_sendMoney_sendTransactionFailureThrowsError() async throws {
    ...
    adamantCoreMock.given(
      .sign(transaction: .any, senderId: .any, keypair: .any, willReturn: "signature"))
    adamantCoreMock.given(
      .encodeMessage(
        .any, recipientPublicKey: .any, privateKey: .any, willReturn: ("message", "nonce")))
    admApiServiceMock.given(
      .sendMessageTransaction(transaction: .any, willReturn: .failure(.accountNotFound)))
    ...
    admApiServiceMock.verify(
      .sendMessageTransaction(
        transaction: .matching {
          $0.type == .chatMessage
            && $0.senderPublicKey
              == "8eefafa8d2f6a51bde207bcdc9029f3725f5d6aaa8f9b8fe3cd6d65d1f315a54"
    ...
    let transactions: [TransferTransaction] = try stack.container.viewContext.fetch(
      TransferTransaction.fetchRequest())
  ...
  func test_sendJustMoney_invalidRecipientThrowsError() async throws {
    ...
    await MainActor.run {
      accountsProviderMock.given(
        .getAccount(byAddress: .any, willThrow: AccountsProviderError.invalidAddress(address: "")))
    }
  ...
  func test_sendJustMoney_invalidRecipientQueriesDummyAndThrowsErrorWhenFails() async throws {
    ...
    await MainActor.run {
      ...
      accountsProviderMock.given(
        .getDummyAccount(
          for: .any, willThrow: AccountsProviderDummyAccountError.invalidAddress(address: "")))
    }
  ...
  func test_sendJustMoney_correctRecipientBadMessageEncodeThrowsError() async throws {
    ...
    // THEN
    adamantCoreMock.verify(
      .encodeMessage(.any, recipientPublicKey: .any, privateKey: .any), count: 0)
    switch result.error as? WalletServiceError {
    case .internalError:
      break
    default:
...
extension AdmWalletServiceTests {
  fileprivate func makeAccount() -> AdamantAccount {
  ...
  fileprivate func setupAccountService() {
    accountServiceMock.given(.account(getter: makeAccount()))
    accountServiceMock.given(.keypair(getter: makeKeypair(passphrase: Constants.passphrase)))
  }

  fileprivate func setupCoreDataEntities(accountPublicKey: String? = nil) -> (
    Chatroom, CoreDataAccount
  ) {
  ...
  fileprivate func createCoreDataAccount(publicKey: String? = nil) -> CoreDataAccount {
  ...
  fileprivate func createChatroom() -> Chatroom {
    let room = Chatroom(context: stack.container.viewContext)

    return room
  }

  fileprivate func makeKeypair(passphrase: String) -> Keypair? {
