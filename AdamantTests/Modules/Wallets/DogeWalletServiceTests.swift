    ...
    func test_createTransaction_enoughMoneyReturnsRealTransaction() async throws {
        ...
        transactionFactoryMock.given(
            .createTransaction(
                ...
                lockTime: .any,
                keys: .any,
                willReturn: Constants.expectedTransaction
            )
        )
...
extension DogeWalletServiceTests {
    fileprivate func makeWallet(address: String = Constants.dogeAddress) throws -> DogeWallet {
    ...
    fileprivate func makeDefaultAddress(address: String = Constants.dogeAddress) throws -> Address {
        try AddressConverterFactory()
            .make(network: DogeMainnet())
            .convert(address: address)
    }

    fileprivate func assertAddressesEqual(_ lhs: Address, _ rhs: Address, file: StaticString = #file, line: UInt = #line) {
