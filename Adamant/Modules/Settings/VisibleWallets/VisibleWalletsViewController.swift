...
extension VisibleWalletsViewController: AdamantVisibleWalletsCellDelegate {
    func delegateCell(
        ...
        if !isInvisible(wallet.tokenUnicID) {
            visibleWalletsService.addToInvisibleWallets(wallet.tokenUnicID)
        } else {
            visibleWalletsService.removeFromInvisibleWallets(wallet.tokenUnicID)
        }
        let walletsTokenUnicID = wallets.map { $0.tokenUnicID }
        visibleWalletsService.setIndexPositionWallets(walletsTokenUnicID, includeInvisible: true)
        visibleWalletsService.setIndexPositionWallets(walletsTokenUnicID, includeInvisible: false)
