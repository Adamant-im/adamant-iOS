    ...
    override func messageContainerSize(for message: MessageType, at indexPath: IndexPath) -> CGSize {
        MainActor.assumeIsolatedSafe {
            ...
            if case let .reply(model) = getMessages()[indexPath.section].fullModel.content {
                ...
                let size =
                    messageSize.width > messageReplySize.width
                    ? messageSize
                    : messageReplySize
                ...
                messageContainerSize.width +=
                    messageInsets.horizontal
                    + additionalWidth
...
extension FixedTextMessageSizeCalculator {
    fileprivate func messageLabelInsets(for message: MessageType) -> UIEdgeInsets {
        MainActor.assumeIsolatedSafe {
            let dataSource = messagesLayout.messagesDataSource
...
extension MessageKind {
    fileprivate var textMessageKind: MessageKind {
        switch self {
