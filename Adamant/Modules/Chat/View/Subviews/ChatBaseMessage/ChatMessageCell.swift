...
final class ChatMessageCell: TextMessageCell, ChatModelView {
    ...
    private var didPerformLongPressAction = false
    ...
    override func layoutMessageContainerView(
        ...
        default:
            if attributes.accessoryViewSize.height > attributes.messageContainerSize.height {
                ...
                origin.y =
                    attributes.cellTopLabelSize.height
...
extension ChatMessageCell {
    ...
    private func handleLongPressToCopy(_ gesture: UILongPressGestureRecognizer) {
        processLongPress(gesture) { [weak self] in
        ...
        switch gesture.state {
            case .began:
                ...
                longPressTask = Task { [weak self] in
                    try? await Task.sleep(interval: 1.5)
                    guard let self = self,
