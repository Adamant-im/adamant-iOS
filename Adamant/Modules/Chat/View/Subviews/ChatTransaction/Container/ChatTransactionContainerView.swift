...
extension ChatTransactionContainerView {
    ...
    fileprivate func updateStatus(_ status: TransactionStatus) {
    ...
    fileprivate func updateOpponentReaction() {
    ...
    fileprivate func getSenderPublicKeyInReaction(for senderAddress: String) -> String? {
        model.reactions?.first(
            where: { $0.sender == senderAddress }
        )?.senderPublicKey
    }

    fileprivate func getReaction(for address: String) -> String? {
        model.reactions?.first(
...
extension ChatTransactionContainerView: GestureHelper {
    @objc private func handleLongPress(_ gesture: UILongPressGestureRecognizer) {
        if !isMacOS, isFailedMessage {
    ...
    private func handleLongPressToCopy(_ gesture: UILongPressGestureRecognizer) {
        processLongPress(
            ...
            perform:     { [weak self] in
                guard let text = self?.model.content.comment else { return }
                self?.copyAction?(text) },
            onGestureBegan: { [weak self] in
                self?.contentView.animatePressDown() },
            onGestureEnded:   { [weak self] in self?.contentView.animatePressUp() }
        )
    }
