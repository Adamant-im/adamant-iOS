...
extension TransactionsStatusServiceCompose {
    fileprivate struct TransferIdentifier: Hashable {
        let transferHash: String
        let blockchain: String
    }

    fileprivate func processCoreDataChange(type: NSFetchedResultsChangeType, transaction: CoinTransaction) {
        switch type {
        case .insert, .update:
    ...
    fileprivate func add(transaction: CoinTransaction) {
        guard let transferIdentifier = transaction.transferIdentifier else { return }

        guard let observer = observers[transferIdentifier] else {
            observers[transferIdentifier] = makeObserver(transaction: transaction)
            return
        }
    ...
    fileprivate func remove(transaction: CoinTransaction) {
        guard
    ...
    fileprivate func makeObserver(transaction: CoinTransaction) -> TxStatusServiceProtocol? {
        guard
    ...
    fileprivate func dismissService(_ service: AnyObject) {
        let key = observers.first { $0.value === service }?.key
        guard let key else { return }
        observers.removeValue(forKey: key)
    }

    fileprivate func saveStatus(
    ...
    fileprivate func getRichTransactionsController() -> NSFetchedResultsController<CoinTransaction> {
...
extension CoinTransaction {
    fileprivate var transferIdentifier: TransactionsStatusServiceCompose.TransferIdentifier? {
        let hash =
            switch self {
