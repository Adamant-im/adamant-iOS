...
extension EthWalletService {
    fileprivate struct EthTransactionInfo: Sendable {
        var details: Web3Core.TransactionDetails?
        var receipt: TransactionReceipt?
    }

    fileprivate enum EthTransactionInfoElement: Sendable {
        case details(Web3Core.TransactionDetails)
        case receipt(TransactionReceipt)
    }

    fileprivate func getTransactionInfo(hash: String, web3: Web3) async throws -> EthTransactionInfo {
    ...
    fileprivate func reportedValue(for transaction: CoinTransaction) -> Decimal? {
