...
extension TransactionStatus {
    ...
    fileprivate var descriptionLocalized: String? {
        switch self {
...
extension String.adamant.alert {
    static var exportUrlButton: String {
        String.localized("TransactionDetailsScene.Share.URL", comment: "Export transaction: 'Share transaction URL' button")
    }
    static var exportSummaryButton: String {
        String.localized("TransactionDetailsScene.Share.Summary", comment: "Export transaction: 'Share transaction summary' button")
    }
}
...
class TransactionDetailsViewControllerBase: FormViewController {
        ...
        let senderRow = DoubleDetailsRow { [weak self] in
            ...
            if let transaction = self?.transaction {
                if let name = self?.senderName {
                    ...
                    $0.value =
                        transaction.senderAddress.isEmpty
    ...
    @objc func share(_ sender: UIBarButtonItem) {
        ...
        if let url = explorerUrl(for: transaction) {
            // URL
            alert.addAction(
                UIAlertAction(title: String.adamant.alert.exportUrlButton, style: .default) { [weak self] _ in
        ...
        if let summary = summary(for: transaction) {
            alert.addAction(
                UIAlertAction(title: String.adamant.alert.exportSummaryButton, style: .default) { [weak self] _ in
                    let text = summary
