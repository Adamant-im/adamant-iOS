    ...
    private(set) var isInitiallySynced: Bool = false {
        didSet {
            NotificationCenter.default.post(
                name: Notification.Name.AdamantChatsProvider.initiallySyncedChanged,
                object: self,
                userInfo: [AdamantUserInfoKey.ChatProvider.initiallySynced: isInitiallySynced]
            )
        }
    }
...
extension AdamantChatsProvider {
    ...
    func getChatRooms(offset: Int?) async {
        ...
        if !isInitiallySynced {
            isInitiallySynced = true
            preLoadChats(array, address: address, withInitial: true)
        } else {
    ...
    func preLoadChats(_ array: [Transaction], address: String, withInitial: Bool = false) {
        let prioritized = array.filter { transaction in
            removedMessages.contains(String(transaction.id))
        }

        let regular: ArraySlice<Transaction> = withInitial
            ? array.filter { transaction in
                !removedMessages.contains(String(transaction.id))
            }.prefix(preLoadChatsCount)
            : []

        let preloadList = prioritized + regular
        guard !preloadList.isEmpty else { return }
...
extension AdamantChatsProvider {
    ...
    func sendFileMessage(
        ...
        guard
            let transactionLocaly = getBaseTransactionFromDB(
    ...
    func setTxMessageStatus(
        ...
        guard
            let transaction = getBaseTransactionFromDB(
                id: txId,
                context: context
            ) as? RichMessageTransaction
        else {
    ...
    private func sendRichMessageLocaly(
        ...
        transaction.transactionStatus =
            walletServiceCompose.getWallet(by: richType) != nil
            ? .notInitiated
            : nil
...
extension AdamantChatsProvider {
    func getChatroomsController() -> NSFetchedResultsController<Chatroom> {
        ...
        let controller = NSFetchedResultsController(
            ...
            managedObjectContext: stack.container.viewContext,
    ...
    func getUnreadMessagesController() -> NSFetchedResultsController<ChatTransaction> {
        ...
        let controller = NSFetchedResultsController(
            fetchRequest: request,
            managedObjectContext: stack.container.viewContext,
            sectionNameKeyPath: "chatroom.partner.address",
            cacheName: nil
        )
        ...
        for (account, transactions) in partners {
            ...
            for trs in transactions {
                    ...
                    )
                {
                    ...
                    let transactionExist =
                        privateChatroom.transactions?.first(where: { message in
