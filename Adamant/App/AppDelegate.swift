...
extension String.adamant {
    ...
    struct application {
        static let deviceTokenSendFailed = String.localized(
            "Application.deviceTokenErrorFormat",
            comment: "Application: Failed to send deviceToken to ANS error format. %@ for error description"
        )
    }
}
...
extension AppDelegate {
    ...
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        if let service = container.resolve(DialogService.self) {
            service.showError(
                withMessage: String.localizedStringWithFormat(NotificationStrings.registrationRemotesFormat, error.localizedDescription),
                supportEmail: true,
                error: error
            )
        }
    ...
    func openDialog(
        ...
        if let chatVCNav = chatListNav.viewControllers.last as? UINavigationController,
        ...
        if let chatVC = chatListNav.viewControllers.last as? ChatViewController,
            chatVC.viewModel.chatroom?.partner?.address == senderAddress
        {
            chatVC.messagesCollectionView.scrollToBottom(animated: true)
            return
