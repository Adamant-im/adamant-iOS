...
class NotificationViewController: UIViewController, UNNotificationContentExtension {
    ...
    func didReceive(_ notification: UNNotification) {
        ...
        let trs: Transaction?
        if let transactionRaw = notification.request.content.userInfo[AdamantNotificationUserInfoKeys.transaction] as? String,
        ...
        if let raw = notification.request.content.userInfo[AdamantNotificationUserInfoKeys.decodedMessage] as? String {
            ...
            guard let passphrase: String = keychainStore.get(passphraseStoreKey),
                let keys = nativeCore.createKeypairFor(passphrase: passphrase, password: .empty),
        ...
        else if let flag = notification.request.content.userInfo[AdamantNotificationUserInfoKeys.partnerNoDislpayNameKey] as? String,
            flag == AdamantNotificationUserInfoKeys.partnerNoDisplayNameValue
        {
            senderName = nil
        }
        // No name, no flag - something broke. Check sender name, if we have a recipient address
            ...
            if let keypair = keypair {
                key = keypair
            } else if let passphrase: String = keychainStore.get(passphraseStoreKey),
                let keypair = nativeCore.createKeypairFor(passphrase: passphrase, password: .empty)
            {
                key = keypair
            } else {
                key = nil
    ...
    private func showError(with message: String? = nil) {
        ...
        warningView.message =
            message.map { .adamant.notifications.error(with: $0) }
