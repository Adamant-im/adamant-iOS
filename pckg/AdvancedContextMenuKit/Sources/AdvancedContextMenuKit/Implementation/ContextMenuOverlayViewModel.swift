...
extension ContextMenuOverlayViewModel {
    fileprivate func calculateContentViewLocation() -> CGPoint {
        .init(
    ...
    fileprivate func isNeedToMoveFromTrailing() -> Bool {
        let maxSize =
            menuSize.width > upperContentSize.width
            ? menuSize
            : upperContentSize
    ...
    fileprivate func calculateOffsetForUpperContentView() -> CGFloat {
        let offset =
            calculateOffsetForContentView()
    ...
    fileprivate func calculateOffsetForContentView() -> CGFloat {
    ...
    fileprivate func shoudScroll() -> Bool {
        guard
            contentViewSize.height
                + menuSize.height
                + minBottomOffset * 2
    ...
    fileprivate func isNeedToMoveFromTop() -> Bool {
        locationOnScreen.y - minContentsSpace - upperContentSize.height < minBottomOffset
    }

    fileprivate func getOffsetToMoveFromTop() -> CGFloat {
        minContentsSpace
            + upperContentSize.height
            + minBottomOffset
    }
    ...
    fileprivate func getOffsetToMoveFromBottom() -> CGFloat {
        UIScreen.main.bounds.height
            - menuSize.height
            - contentViewSize.height
            - minBottomOffset
