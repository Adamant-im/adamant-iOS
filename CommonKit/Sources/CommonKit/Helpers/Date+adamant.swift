...
extension Date {
    // MARK: - Constants
    public static let adamantNullDate = Date(timeIntervalSince1970: .zero)

    // MARK: - Humanized dates

    /// Returns readable date with time.
    public func humanizedDateTime(withWeekday: Bool = true) -> String {
    ...
    public func humanizedDateTimeFull() -> String {
        let formatter = defaultFormatter
    ...
    public func humanizedDay(useTimeFormat: Bool) -> String {
        let dateString: String
        if isToday {  // Today
            if useTimeFormat {
                let formatter = defaultFormatter
    ...
    public func humanizedTime() -> (string: String, expireIn: TimeInterval?) {
...
extension Date {
    ...
    fileprivate static var formatterWeekDayMonth: DateFormatter {
    ...
    fileprivate static var formatterDayMonth: DateFormatter {
        let formatter = DateFormatter()
        if let localeRaw = UserDefaults.standard.string(forKey: StoreKey.language.languageLocale) {
            formatter.locale = Locale(identifier: localeRaw)
        }
    ...
    fileprivate func elapsedTime(from date: Date) -> String {
    ...
    enum dateChatList {
        ...
        static func minutes(_ minutes: Int) -> String {
            return String.localizedStringWithFormat(
                .localized("Chats.Date.For.Minutes", comment: "Date chats: Duration in minutes if longer than one."),
                minutes
            )
        }
        ...
        static func seconds(_ seconds: Int) -> String {
            return String.localizedStringWithFormat(
                .localized("Chats.Date.For.Seconds", comment: "Date chats: Duration in seconds if longer than one."),
                seconds
            )
        }
    }
