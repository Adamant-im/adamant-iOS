...
>: StreamSendableActor where P.Output: Sendable, P.Failure: Sendable {
    ...
    nonisolated public let streamSender:
        AsyncStreamSender<
...
extension SendablePublisher {
    fileprivate func subscribe(_ continuation: AsyncThrowingStream<P.Output, any Error>.Continuation) {
        let id = UUID()
    ...
    fileprivate func removeSubscription(id: UUID) {
