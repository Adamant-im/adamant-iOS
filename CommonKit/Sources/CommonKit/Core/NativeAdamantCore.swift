...
public final class NativeAdamantCore: AdamantCore {
    ...
    public func encodeValue(_ value: [String: Any], privateKey privateKeyHex: String) -> (message: String, nonce: String)? {
        ...
        let padded: String =
            String.random(length: Int(arc4random_uniform(10)), alphabet: "abcdefghijklmnopqrstuvwxyz")
            + AdamantUtilities.JSONStringify(value: data as AnyObject)
            + String.random(length: Int(arc4random_uniform(10)), alphabet: "abcdefghijklmnopqrstuvwxyz")
...
extension String {
    ...
    public static func random(length: Int = 32, alphabet: String = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") -> String {
        let upperBound = UInt32(alphabet.count)
        return String(
            (0..<length).map { _ -> Character in
...
extension SignableTransaction {
    ...
    fileprivate var bytes: [UInt8] {
        return
            typeBytes + timestampBytes + senderPublicKeyBytes + requesterPublicKeyBytes + recipientIdBytes + amountBytes + assetBytes + signatureBytes
            + signSignatureBytes
    }

    fileprivate var typeBytes: [UInt8] {
        return [UInt8(type.rawValue)]
    }

    fileprivate var timestampBytes: [UInt8] {
        return ByteBackpacker.pack(UInt32(timestamp), byteOrder: .littleEndian)
    }

    fileprivate var senderPublicKeyBytes: [UInt8] {
        return senderPublicKey.hexBytes()
    }

    fileprivate var requesterPublicKeyBytes: [UInt8] {
        return requesterPublicKey?.hexBytes() ?? []
    }

    fileprivate var recipientIdBytes: [UInt8] {
        guard
            let value = recipientId?.replacingOccurrences(of: "U", with: ""),
            let number = UInt64(value)
        else { return Bytes(count: 8) }
        return ByteBackpacker.pack(number, byteOrder: .bigEndian)
    }

    fileprivate var amountBytes: [UInt8] {
    ...
    fileprivate var signatureBytes: [UInt8] {
        return []
    }

    fileprivate var signSignatureBytes: [UInt8] {
        return []
    }

    fileprivate var assetBytes: [UInt8] {
        switch type {
        case .chatMessage:
