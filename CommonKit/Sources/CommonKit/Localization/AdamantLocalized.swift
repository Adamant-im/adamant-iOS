...
extension String {
    ...
    public static func localized(_ key: String, comment: String = .empty) -> String {
        guard let languageRaw = UserDefaults.standard.string(forKey: StoreKey.language.language),
            !languageRaw.isEmpty,
            languageRaw != Language.auto.rawValue,
...
extension String.adamant {
    ...
    public enum sharedErrors {
        ...
        public static func accountNotFound(_ account: String) -> String {
            String.localizedStringWithFormat(
                .localized("Error.AccountNotFoundFormat", comment: "Shared error: Account not found error. Using %@ for address."),
                account
            )
        }
        ...
        public static func admNodeErrorMessage(_ coin: String) -> String {
            String.localizedStringWithFormat(
                .localized("ApiService.InternalError.NoAdmNodesAvailable", comment: "No active ADM nodes to fetch the partner's %@ address"),
                coin
            )
        }
